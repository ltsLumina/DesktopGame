...
public static class VFavorites
{
    static void OnGUI(object dummy)
    {
        ...
        Rect footerRect = totalRect.SetHeightFromBottom(21);
        Rect bodyRect   = totalRect.SetHeight(totalRect.height - footerRect.height);
        ...
        void dragndrop()
        {
            ...
            {
                string guid = DragAndDrop.objectReferences.FirstOrDefault()?.GetGuid() ?? "";
                if (guid != "") InitDrag(guid);
            }

            if (dragging) DragAndDrop.visualMode = draggingGuidFromList ? DragAndDropVisualMode.Move : DragAndDropVisualMode.Copy;

            if (dragging && eType == EventType.DragPerform) AcceptDrag();
        ...
        void mouseDown()
        {
            ...
            if (e.mouseUp()) mousePressed = false;
        }
        ...
        void gaps_()
        {
            ...
            if (eType == EventType.Layout)
                for (int i = 0; i < gaps.Count; i++) { gaps[i] = Lerp(gaps[i], dragging && i == draggedGuidInsertAt ? rowHeight : 0, lerpSpeed); }
        }
        ...
        void footer()
        {
            void background()
            {
                Color footerCol = EditorGUIUtility.isProSkin ? Greyscale(.25f) : Greyscale(.81f);
                Color lineCol   = EditorGUIUtility.isProSkin ? Greyscale(.155f) : Greyscale(.7f);
            ...
            void scaleSlider()
            {
                Rect rect = footerRect.SetWidthFromRight(60).MoveX(-15).MoveY(2f).SetHeightFromMid(22);

                float prev = rowScale;
                float cur  = GUI.HorizontalSlider(rect, rowScale, .6f, 1.4f);

                cur = Mathf.RoundToInt(cur * 10) / 10f;

                if (prev != cur) rowScale = cur;
            }
            ...
            void mouseDown()
            {
                ...
                if (e.mouseDown()) e.Use();
            }

            void undoButton()
            {
                Rect rect = footerRect.SetWidth(30).MoveX(5).MoveY(.5f).SetHeightFromMid(14);
        ...
        void rows()
        {
            void row(float y, string guid, float selectedColT = 0, bool isDragged = false)
            {
                ...
                bool rowHovered = rowRect.IsHovered();

                if (rowHovered && mousePressed && !dragging) selectedColT = 1;

                string path     = guid.ToPath();
                ...
                void background()
                {
                    Color rowColor = Color.Lerp(evenRowCol, oddRowCol, Mathf.PingPong(y, rowHeight) / rowHeight);
                ...
                void icon()
                {
                    ...
                    if (tex == null && obj != null) tex = AssetPreview.GetMiniTypeThumbnail(obj.GetType());

                    if (tex == null) tex = Texture2D.grayTexture;

                    Rect rect = rowRect.SetWidth(20).SetWidthFromRight(iconHeight).SetHeightFromMid(iconHeight).MoveX(5).Resize(-2);
                ...
                void name_()
                {
                    Rect rect = rowRect.SetHeightFromMid(20).MoveX(29);
                    GUI.Label(rect, name);
                }
                ...
                void crossButton()
                {
                    ...
                    Rect rect = rowRect.SetWidthFromRight(16).MoveX(-11).SetHeightFromMid(16);

                    bool crossHovered = rect.Resize(-3).IsHovered();

                    Color cNorm    = Greyscale(.45f);
                    Color cHovered = EditorGUIUtility.isProSkin ? Greyscale(.8f) : cNorm;
                    ...
                    if (crossHovered) mousePressed = false;
                ...
                void click()
                {
                    ...
                    if (AssetDatabase.IsValidFolder(path)) { OpenFolder(path); }
                    else if (EditorApplication.timeSinceStartup - lastClickTime < .3f && (path.GetExtension() == ".cs"    || path.GetExtension() == ".compute" || path.GetExtension() == ".shader" ||
                                                                                          path.GetExtension() == ".cginc" || path.GetExtension() == ".json")) { AssetDatabase.OpenAsset(guid.LoadGuid()); }
                    else
                    {
            ...
            for (int i = 0; i < guids.Count; i++)
            {
                ...
                if (!lerpingAcceptedDrag || acceptedDragInd != i) row(lastRect.y, guids[i]);
                else if (eType == EventType.Repaint) accepterDragTargetY = lastRect.y;
            }
            ...
            if (dragging) row(draggedGuidLocalY, draggedGuid, 1, true);
            ...
            if (lerpingAcceptedDrag)
            {
                ...
                if (accepterDragY.Approx(accepterDragTargetY)) lerpingAcceptedDrag = false;
            }
    ...
    {
        get => Vector2.up * EditorPrefs.GetFloat(prefsScrollKey);
        set => EditorPrefs.SetFloat(prefsScrollKey, value.y);
    }
    static float rowScale
    {
        get => EditorPrefs.GetFloat(prefsScaleKey, 1);
        set => EditorPrefs.SetFloat(prefsScaleKey, value);
    }
    ...
    static void AcceptDrag()
    {
        ...
        if (!draggingGuidFromList) RecordGuidsUndo();
    ...
    static List<float> gaps = new ();
    static List<string> guids = new ();
    static Stack<List<string>> guidsForUndoStack = new ();
    ...
    static void UpdateOnGUIWrapping()
    {
        ...
        bool anyBrowserHovered     = EditorWindow.mouseOverWindow?.GetType() == t_BrowserWindow;
        bool wrappedBrowserHovered = EditorWindow.mouseOverWindow            == wrappedBrowser;

        bool shortcutPressed = curEvent.modifiers == EventModifiers.Alt;

        if (!shortcutPressed) mouseLeftWhileDragging = false;
    ...
    static void WrapBrowserOnGUI(EditorWindow browser)
    {
        ...
        object   hostView    = fi_m_Parent.GetValue(browser);
        Delegate newDelegate = mi_OnGUI.CreateDelegate(t_EditorWindowDelegate, hostView);
    ...
    static void UnwrapBrowserOnGUI()
    {
        lerpingAcceptedDrag = false;
        dragging            = false;
        for (int i = 0; i < gaps.Count; i++) { gaps[i] = 0; }

        object hostView = fi_m_Parent.GetValue(wrappedBrowser);
