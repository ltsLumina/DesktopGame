...
public static class VFolders
{
    static void ItemGUI(string guid, Rect rect)
    {
        ...
        if (!foldersFirstInited) UpdateFoldersFirst();

        bool isSingleLine = rect.height == 16;
        bool isListView   = isSingleLine && rect.x == 14;

        if (!AssetDatabase.IsValidFolder(guid.ToPath()))
        {
            if (eType == EventType.Repaint && isSingleLine) listviewAssetRectX = GUIUtility.GUIToScreenPoint(rect.position).x;
        ...
        void folderData_()
        {
            if (!data.folderDatasByGuid.ContainsKey(guid)) data.folderDatasByGuid.Add(guid, new (guid));

            folderData = data.folderDatasByGuid[guid];

            if (folderData.autoIconDirty) UpdateAutoIcon(folderData);
        }
        ...
        void rects()
        {
            if (isSingleLine)
            {
                ...
                if (isListView) folderIconRect.x += 3;
            }
            else
            {
            ...
            float iconSizeMin   = 14f;

            var iconOffsetMax = new Vector2(18, 15);
            int iconSizeMax   = 30;

            float t                                                  = (folderIconRect.width - 16) / (64 - 16);
            if (Application.platform == RuntimePlatform.OSXEditor) t = t.Clamp01();

            Vector3 iconOffset = Lerp(iconOffsetMin, iconOffsetMax, t);
            float   iconSize   = Lerp(iconSizeMin, iconSizeMax, t);
        ...
        void color()
        {
            ...
            bool isEmpty = !Directory.EnumerateFileSystemEntries(guid.ToPath()).Any();
        ...
        void icon()
        {
            ...
            Texture texture = EditorGUIUtility.IconContent(folderData.icon).image;

            void material()
            {
                if (outlineMaterial) return;

                outlineMaterial = new (Shader.Find("Hidden/VFoldersOutline"));
            ...
            void outline()
            {
                ...
                Rect outlineRect = iconRect.Resize(rect.height >= 70 && EditorGUIUtility.pixelsPerPoint >= 2 ? -1f / EditorGUIUtility.pixelsPerPoint : 0).AlignToPixelGrid();
            ...
            void background()
            {
                ...
                for (int i = 0; i < iconRect.size.x / 2; i++) { EditorGUI.DrawPreviewTexture(iconRect.Resize(i * 1f + 1), texture, outlineMaterial); }
        ...
        void altDrag() // has to be set up manually before altClick because altClick will use() mouseDown event to prevent selection change
        {
            ...
            void mouseDrag()
            {
                ...
                { AssetDatabase.LoadAssetAtPath<Object>(guid.ToPath()) };
        ...
        void altClick()
        {
            ...
            void mouseUp()
            {
                ...
                if (VFoldersIconEditor.currentlyOpenedForGuid == guid)
                {
                    VFoldersIconEditor.instance?.Close();
                    return;
                }
    ...
    static void UpdateAutoIcon(FolderData folderData)
    {
        ...
        string path = folderData.guid.ToPath();

        if (!Directory.Exists(path)) return;

        IEnumerable<Type> types = Directory.GetFiles(path, "*.*").Select(r => AssetDatabase.GetMainAssetTypeAtPath(r)).Where(r => r != null);
        ...
        if (type == typeof(SceneAsset)) folderData.autoIcon = "SceneAsset Icon";

        else if (type == typeof(GameObject)) folderData.autoIcon = "Prefab Icon";

        else if (type == typeof(Material)) folderData.autoIcon = "Material Icon";

        else if (type == typeof(Texture)) folderData.autoIcon = "Texture Icon";

        else if (type.BaseType == typeof(ScriptableObject)) folderData.autoIcon = "ScriptableObject Icon";

        else if (type == typeof(TerrainData)) folderData.autoIcon = "TerrainData Icon";

        else if (type == typeof(AudioClip)) folderData.autoIcon = "AudioClip Icon";

        else if (type == typeof(Shader)) folderData.autoIcon = "Shader Icon";

        else if (type == typeof(ComputeShader)) folderData.autoIcon = "ComputeShader Icon";

        else if (type == typeof(MonoScript) || type == typeof(AssemblyDefinitionAsset) || type == typeof(AssemblyDefinitionReferenceAsset)) folderData.autoIcon = "cs Script Icon";
    }
    ...
    static void UpdateFoldersFirst()
    {
        ...
        Type t_ProjectBrowser = typeof(Editor).Assembly.GetType("UnityEditor.ProjectBrowser");

        foreach (Object browser in Resources.FindObjectsOfTypeAll(t_ProjectBrowser))
        {
            if (foldersFirstInitedForBrowsers.Contains(browser)) continue;

            object m_ListArea = t_ProjectBrowser.GetField("m_ListArea", maxBindingFlags).GetValue(browser);
    ...
    static List<Object> foldersFirstInitedForBrowsers = new ();
