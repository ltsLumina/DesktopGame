...
public class VFoldersIconEditor : CustomPopupWindow
{
    void OnGUI()
    {
        ...
        Color bgHovered  = Greyscale(1, .3f);
        ...
        void icons()
        {
            ...
            {
                if (icon == initIcon) rect.Draw(bgSelected);
                ...
                if (e.mouseDown() && rect.Resize(-1).IsHovered()) Close();
            ...
            for (int i = 0; i < iconRows.Length; i++)
            {
                ...
                Rect iconRect = lastRect.SetWidth(buttonSize).SetHeightFromMid(buttonSize).MoveX(rowStartX);

                foreach (string icon in iconRows[i])
                {
        ...
        void colors_()
        {
            ...
            {
                if (iColor == initIColor) rect.Draw(bgSelected);
            ...
            if (hoveredIColor != -1) folderData.iColor = hoveredIColor;
            else folderData.iColor                     = initIColor;
        }
    ...
    void OnDestroy()
    {
        ...
        {
            if (currentlyOpenedForGuid == wasJustDestroyedForGuid) currentlyOpenedForGuid = "";
    ...
    public static Color GetColor(int iColor)
    {
        ...
        {
            Color color = EditorGUIUtility.isProSkin ? HSLToRGB((i - 0f) / (colors.Length - 1), .9f, .65f) : HSLToRGB((i - 0f) / (colors.Length - 1), .9f, .6f);
            color   *= 1.1f;
            color.a =  1;
