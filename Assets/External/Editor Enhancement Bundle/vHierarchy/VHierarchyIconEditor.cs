...
public class VHierarchyIconEditor : CustomPopupWindow
{
    void OnGUI()
    {
        ...
        void colors_()
        {
            ...
            void colorButton(Rect rect, int iColor)
            {
                ...
                else { rect.Resize(4).Draw(GetColor(iColor) * (EditorGUIUtility.isProSkin ? 1.3f : 1)); }
            }
            ...
            if (hoveredIColor != -1) goData.iColor = hoveredIColor;
            else goData.iColor                     = initIColor;
    ...
    public static Color GetColor(int iColor)
    {
        ...
        {
            Color color = EditorGUIUtility.isProSkin ? HSLToRGB((i + 1f) / (colors.Length - 1), .4f, .34f) : HSLToRGB((i + 1f) / (colors.Length - 1), .6f, .8f);
            color   *= 1.1f;
            color.a =  1;
