...
public class VISerializedDictionaryDrawer : PropertyDrawer
{
    public override void OnGUI(Rect rect, SerializedProperty prop, GUIContent label)
    {
        ...
        void header()
        {
            Rect headerRect = indentedRect.SetHeight(EditorGUIUtility.singleLineHeight);

            void foldout()
            {
                Rect fullHeaderRect = headerRect.MoveX(3).AddWidthFromRight(17);

                if (fullHeaderRect.IsHovered()) fullHeaderRect.Draw(Greyscale(1, .07f));

                GUI.color = Color.clear;
                if (GUI.Button(fullHeaderRect.AddWidth(-50), "")) prop.isExpanded = !prop.isExpanded;
                GUI.color = Color.white;

                Rect triangleRect = rect.SetHeight(EditorGUIUtility.singleLineHeight);
            ...
            void count() => kvpsProp.arraySize = EditorGUI.DelayedIntField(headerRect.SetWidthFromRight(48 + EditorGUI.indentLevel * 15), kvpsProp.arraySize);
            ...
            void repeatedKeysWarning()
            {
                ...
                for (int i = 0; i < kvpsProp.arraySize; i++) { hasRepeated |= kvpsProp.GetArrayElementAtIndex(i).FindPropertyRelative("isKeyRepeated").boolValue; }

                if (!hasRepeated) return;

                Rect warningRect = headerRect.AddWidthFromRight(-prop.displayName.GetLabelWidth()).MoveX(3);
    ...
    {
        SerializedProperty kvpProp   = kvpsProp.GetArrayElementAtIndex(index);
        ...
        float getPropHeight(SerializedProperty prop)
        {
            ...
            foreach (SerializedProperty childProp in GetChildren(prop, false)) { height += EditorGUI.GetPropertyHeight(childProp) + 1; }
    ...
    void DrawListElement(Rect rect, int index, bool isActive, bool isFocused)
    {
        ...
        {
            if (IsSingleLine(prop)) EditorGUI.PropertyField(rect_.SetHeight(EditorGUIUtility.singleLineHeight), prop, GUIContent.none);
            else
        ...
        void rects()
        {
            ...
            float dividerPos = dividerPosProp.floatValue;

            Rect fullRect = rect.AddWidthFromRight(-1).AddHeightFromMid(-2);
        ...
        void key()
        {
            ...
            if (kvpProp.FindPropertyRelative("isKeyRepeated").boolValue) GUI.Label(keyRect.SetWidthFromRight(20).SetHeight(20).MoveY(-1), EditorGUIUtility.IconContent("Warning"));
        }

        void value() => drawProp(valueRect, valueProp);
        ...
        void divider()
        {
            ...
            {
                if (eType == EventType.MouseDown) isDividerDragged = true;

                if (eType == EventType.MouseUp || eType == EventType.MouseMove || eType == EventType.MouseLeaveWindow) isDividerDragged = false;
            }

            if (isDividerDragged && eType == EventType.MouseDrag) dividerPosProp.floatValue = Mathf.Clamp(dividerPosProp.floatValue + e.delta.x / rect.width, .2f, .8f);
        }
    ...
    public void SetupList(SerializedProperty prop)
    {
        ...
        list                         = new (kvpsProp.serializedObject, kvpsProp, true, false, true, true);
