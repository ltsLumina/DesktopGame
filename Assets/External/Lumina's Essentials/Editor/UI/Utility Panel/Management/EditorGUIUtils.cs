...
internal static class EditorGUIUtils
{
    ...
    internal static void SetGUIStyles()
    {
        ...
        // GUIContent
        safeModeWarningContent = new ("Safe Mode", "If disabled, the user is able to perform certain operations that could potentially be dangerous or cause unintended behaviour.");

        createDefaultProjectContent = new
            ("Create Default Project Structure", "Creates a project structure with the recommended default folders such as Art, Scripts, etc. \nRequires Safe Mode to be disabled in the settings.");

        configureImagesContent = new ("Image Settings Converter", "Configures the default settings for images. \n" + "This is useful for when you want to import images with the same settings every time.");

        createSubfolderContent = new // Deprecated. Kept for reference.
            ("Create Subfolder", "Creates a subfolder in the selected folder.");

        enterPlaymodeOptionsContent = new ("Enter Playmode Options", "Enabling \"Enter Playmode Options\" improves Unity's workflow by significantly reducing the time it takes to enter play mode.");
        ...
        checkForUpdatesContent = new ("Check for Updates", "Checks if a new version of Lumina's Essentials is available. \n(Requires internet connection)");

        openKnownIssuesContent = new ("Known Issues", "Opens the known issues page on the GitHub repository. \nPlease add any issues you encounter to the list.");
    }
    ...
    { // If the user is in play mode, display a message telling them that the utility panel is not available while in play mode.
        ...
        GUILayout.Space(40);
        using (new GUILayout.HorizontalScope()) { GUILayout.Label("The Utility Panel \nis disabled while in play mode.", wrapCenterLabelStyle, GUILayout.ExpandWidth(true)); }
        GUILayout.Space(40);
        return true;
    }

    internal static string Tooltip(string message = default) => message;
