...
#pragma warning restore CS0414 // Field is assigned but its value is never used
    ...
    { // Button that creates a default project directory structure
        if (GUILayout.Button(createDefaultProjectContent, GUILayout.Height(35)))
        {
            if (!SafeMode)
            {
                ...
                if (EditorUtility.DisplayDialog("Confirmation", "Are you sure you want to create the default project structure?", "Yes", "No")) { CreateProjectStructure(); }
                else
                {
                    SafeMode = true;
            ...
            else
            {
                ...
                ("Warning!", "Safe Mode has not been disabled and is required to create the default project structure. \nThis is done in order to prevent you from creating folders unintentionally.",
    ...
    { // Display the image configuration options
        ...
        var spriteModeContent = new GUIContent("Sprite-Sheet", "Sets the recommended settings for sprite-sheets \n" + "This will set the sprite mode to multiple, filter mode to point, and compression to uncompressed.");
