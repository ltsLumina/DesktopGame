...
internal sealed partial class UtilityPanel : EditorWindow
{
        ...
        get
        {
            ...
            switch (showTooltip)
            {
                case true when ModuleManager.FinishedImporting:
                    message = ModuleManager.ImportStatus ? "Import was successful!" : "Import was unsuccessful. Please try again.";
                    break;

                default: {
                    if (checkedForUpdates) message = EditorPrefs.GetBool("UpToDate") ? "You are up to date!" : "There is a new version available!";
                    else message                   = "Welcome to Lumina's Essentials!";
    ...
    void OnEnable()
    {
        ...
        void Initialization()
        {
            ...
            if (!EditorPrefs.GetBool("UpToDate")) EssentialsDebugger.LogWarning("There is a new version available!" + "\nPlease update to the latest version for the latest features.");
    ...
    void DrawSetupGUI()
    {
        ...
        if (SetupRequired)
        {
            ...
            using (new GUILayout.VerticalScope(GUI.skin.box)) { GUILayout.Label("SETUP REQUIRED", setupLabelStyle); }
...
Check out the ""Utilities"" tab to access the various workflow-enhancing features that this package provides.";
        ...
        using (new GUILayout.HorizontalScope())
        {
            if (GUILayout.Button(openDocumentationContent, GUILayout.Width(buttonSize), GUILayout.Height(40))) Application.OpenURL("https://github.com/ltsLumina/Lumina-Essentials");

            if (GUILayout.Button(openChangeLogContent, GUILayout.Width(buttonSize), GUILayout.Height(40))) Application.OpenURL("https://github.com/ltsLumina/Lumina-Essentials/releases/latest");
        }
        ...
        using (new GUILayout.HorizontalScope())
        {
            ...
            if (GUILayout.Button(checkForUpdatesContent, GUILayout.Width(buttonSize), GUILayout.Height(40)))
            {
                ...
                else { EssentialsDebugger.Log("You are up to date!"); }
            }

            if (GUILayout.Button(openKnownIssuesContent, GUILayout.Width(buttonSize), GUILayout.Height(40))) Application.OpenURL("https://github.com/ltsLumina/Lumina-Essentials/issues");
        }
        #endregion
        GUILayout.Space(3);
